...
class DynamixelMultiDriver : public dynamixel_driver::DynamixelDriver
{
 ...
 private:
  ...
  dynamixel::GroupSyncWrite *groupSyncWriteMaxMinPosition_;
  ...
  dynamixel::GroupSyncWrite *groupSyncWriteKpPositionGain_;
  dynamixel::GroupSyncWrite *groupSyncWriteReturnLevel_;
  dynamixel::GroupSyncWrite *groupSyncWriteOperatingMode_;
  dynamixel::GroupSyncWrite *groupSyncWriteShutdown_;

  dynamixel::GroupSyncRead  *groupSyncReadPosition_;
  dynamixel::GroupSyncRead  *groupSyncReadCurrent_;
  dynamixel::GroupSyncRead  *groupSyncReadKdPositionGain_;
 ...
 public:
  ...
  bool syncWritePosVelAcc(std::vector<uint32_t> pos, std::vector<uint32_t> vel,std::vector<uint32_t> acc);
  bool syncWritePosVel1(std::vector<uint32_t> pos, std::vector<uint32_t> vel);
  bool syncWritePosVel2(std::vector<uint32_t> pos, std::vector<uint32_t> vel);
  bool syncWriteKpPositionGain(std::vector<uint16_t> kp);
  bool syncWriteReturnLevel(std::vector<uint8_t> level);
  bool syncWriteOperatingMode(std::vector<uint8_t> mode);
  bool syncWriteShutdown(std::vector<uint8_t> mode);
  bool syncWriteMaxMinPosition(std::vector<uint32_t> maxPos,std::vector<uint32_t> minPos);
  
  bool syncReadPosition(std::vector<uint32_t> &pos);
  bool syncReadCurrent(std::vector<uint32_t> &curr);
};
}
